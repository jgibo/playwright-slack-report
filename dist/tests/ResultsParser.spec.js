...
const ResultsParser_1 = require("../src/ResultsParser");
...
test.describe('ResultsParser', () => {
    test('determines correct browser based on project config', async ({ testData, }) => {
        const resultsParser = new ResultsParser_1.default({ separateFlakyTests: false });
    ...
    test('parses multiple failures successfully', async ({ testData }) => {
        const resultsParser = new ResultsParser_1.default({ separateFlakyTests: false });
        resultsParser.addTestResult(testData.suites[0].suites[0].title, testData.suites[0].suites[0].tests[0], []);
        resultsParser.addTestResult(testData.suites[0].suites[0].title, testData.suites[1].suites[0].tests[0], []);
        const results = await resultsParser.getParsedResults();
        (0, test_1.expect)(results.failures).toEqual([
            {
    ...
    test('separates flaky tests from successful tests when flaky option is enabled', async ({ testData, }) => {
        const resultsParser = new ResultsParser_1.default({ separateFlakyTests: true });
        // add the flaky test run 1 - failed
        resultsParser.addTestResult(testData.suites[0].suites[0].title, testData.suites[2].suites[0].tests[0], []);
        // add the flaky test run 2 - passed
        resultsParser.addTestResult(testData.suites[0].suites[0].title, testData.suites[2].suites[0].tests[1], []);
        // add a non-flaky passed test
        resultsParser.addTestResult(testData.suites[0].suites[0].title, testData.suites[2].suites[0].tests[2], []);
        const results = await resultsParser.getParsedResults();
        (0, test_1.expect)(results.passed).toBe(1);
        (0, test_1.expect)(results.failures.length).toBe(0);
        (0, test_1.expect)(results.flaky).toBe(1);
    });
