...
const SlackReporter_1 = require("../src/SlackReporter");
const test = test_1.test.extend({
        ...
        projects: [
            {
                ...
                _screenshotsDir: '/home/ry/_repo/playwright-slack-report/__screenshots__/linux/chrome',
                testIgnore: [],
                testMatch: '**/?(*.)@(spec|test).*',
                timeout: 0,
                use: {
    ...
    fakeSlackReporter: async ({}, use) => {
        const slackReporter = new SlackReporter_1.default();
        await use(slackReporter);
    },
});
test.describe('SlackReporter - onEnd()', () => {
    test('onEnd should halt when configured to stop when no failures are encountered', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        test.fixme();
        ...
        (0, test_1.expect)(fakeSlackReporter.logs.includes('⏩ Slack reporter - no failures found')).toBeTruthy();
    });
    test('onEnd should halt when the preChecks fail', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        ...
        fakeSlackReporter.onBegin(cloneFullConfig, suite);
        await fakeSlackReporter.onEnd();
        (0, test_1.expect)(fakeSlackReporter.logs.includes('❌ Neither slack webhook url, slackOAuthToken nor process.env.SLACK_BOT_USER_OAUTH_TOKEN were found')).toBeTruthy();
    });
});
test.describe('SlackReporter - preChecks()', () => {
    ...
    test('okToProceed flag is set when no errors encountered', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        ...
        (0, test_1.expect)(result.okToProceed).toBeTruthy();
    });
    test('okToProceed flag is set to false when SLACK_BOT_USER_OAUTH_TOKEN is not set', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        delete process.env.SLACK_BOT_USER_OAUTH_TOKEN;
        const cloneFullConfig = JSON.parse(JSON.stringify(fullConfig));
        cloneFullConfig.reporter = [
            [
    ...
    test('okToProceed flag is set to false when the reporter is turned off', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        const cloneFullConfig = JSON.parse(JSON.stringify(fullConfig));
        cloneFullConfig.reporter = [
            [
                '/home/ry/_repo/playwright-slack-report/src/SlackReporter.ts',
    ...
    test('okToProceed flag is set to false when an invalid sendResults value is provided', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        const cloneFullConfig = JSON.parse(JSON.stringify(fullConfig));
        cloneFullConfig.reporter = [
            [
                '/home/ry/_repo/playwright-slack-report/src/SlackReporter.ts',
    ...
    test('okToProceed flag is set to false when no channel id supplied', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        const cloneFullConfig = JSON.parse(JSON.stringify(fullConfig));
        cloneFullConfig.reporter = [
            [
                '/home/ry/_repo/playwright-slack-report/src/SlackReporter.ts',
    ...
    test('okToProceed flag is set to false when the custom layout provided is not a Function', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        const cloneFullConfig = JSON.parse(JSON.stringify(fullConfig));
        cloneFullConfig.reporter = [
            [
                '/home/ry/_repo/playwright-slack-report/src/SlackReporter.ts',
